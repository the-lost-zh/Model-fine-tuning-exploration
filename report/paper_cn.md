...
| 数据集 | 类别数 | 训练集 | 测试集 | 特点 |
|--------|--------|--------|--------|------|
| CUB-200-2011 [7] | 200 | 5,994 | 5,794 | 鸟类细粒度分类标准基准 |
| Oxford Flowers-102 [8] | 102 | 1,020 | 6,149 | 极小训练集的挑战场景 |
| Stanford Cars [9] | 196 | 8,144 | 8,041 | 车型识别，类别数多 |

所有图像统一缩放至224×224，使用ImageNet均值和标准差进行归一化。CUB-200-2011和Stanford Cars采用官方train/test划分，Flowers-102将官方训练集和验证集合并为训练集。三个数据集涵盖了不同的训练数据规模（1,020~8,144张），可分别考察各方法在极小样本、中等样本和常规样本下的适应能力。

### 3.2 实验平台与设置

所有实验在一台配备8张NVIDIA RTX 4090（24GB显存）的服务器上完成，每张GPU一次仅运行一个实验，通过`CUDA_VISIBLE_DEVICES`进行隔离。软件环境为Python 3.12、PyTorch 2.5.1（CUDA 12.4）、torchvision 0.20.1和timm 0.9+。ViT-B/16预训练权重从HuggingFace Hub加载（模型名：`vit_base_patch16_224.augreg_in1k`），基于ImageNet-21K预训练。训练使用AdamW优化器，单个实验根据方法和数据集不同耗时约30秒至3分钟（CUB200数据集为参考）。所有实验结果通过3个随机种子（42、123、456）取均值±标准差以评估统计显著性。完整代码和实验配置已开源在GitHub（见§4.3）。
...
### 3.7 稀疏度分析（SSF-Sparse / Gate-LoRA）
...
本课题针对视觉Transformer模型在下游任务上全参数微调成本过高的问题，系统对比了八种参数高效微调方法，并提出了两种创新设计。课题的核心意义在于：（1）为视觉PEFT领域提供了目前最全面的多方法、多数据集系统对比基准；（2）揭示了不同PEFT策略在网络深度上的互补特性（SSF擅长浅层特征归一化，LoRA擅长深层语义适应），为未来混合方法设计提供了理论依据；（3）提出的Gate-LoRA以共享门控实现LoRA+SSF融合，在全部三个数据集上取得最优准确率，证明了"少即是多"的设计哲学——通过精巧的模块设计而非堆叠参数来提升性能。

本文在ViT-B/16上完成了99组实验，核心结论如下：
...
1. **数据集局限**：仅在三个FGVC数据集上验证，原因是FGVC是评估PEFT方法细粒度判别能力的典型场景；未来可在更多样化的任务（目标检测、语义分割）上测试方法的泛化性。
2. **模型规模局限**：仅测试了ViT-B/16（86M参数），原因是8×RTX 4090环境下ViT-Large（307M）的全参数微调显存不足；Gate-LoRA在更大模型上的可扩展性有待验证。
3. **门控稀疏度未充分利用**：SSF-Sparse和Gate-LoRA的稀疏度分析目前仅作为观察性结论，原因在于稀疏度与精度的量化关系需要更大规模的λ扫描实验；未来可探索基于稀疏度的模型剪枝策略。
4. **混合策略仅探索了一种组合**：LoRA+SSF之外，BitFit+AdaptFormer等组合也可能有效，原因是不同PEFT策略之间可能存在更复杂的协同效应，值得在未来工作中系统探索。
