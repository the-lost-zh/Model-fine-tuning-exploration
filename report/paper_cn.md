...
1. **系统对比**：在三个FGVC数据集上，对7种微调方法进行全维度对比（准确率、参数量、训练时间、GPU显存），揭示了视觉任务上PEFT方法的性能排序。
...
本文采用ViT-B/16（Vision Transformer Base, patch size 16）作为基础模型。该模型包含12层Transformer blocks，隐藏维度768，12个注意力头，总参数量约86M。使用ImageNet-21K预训练权重（从timm库加载）。
...
图1展示了各方法在不同训练集比例（10%, 25%, 50%, 100%）下的表现。关键观察：
...
本文系统对比了八种微调方法在ViT-B/16上的表现，并提出了两种创新方法SSF-Sparse和Gate-LoRA。在三个FGVC数据集上的实验表明：

1. PEFT方法在多数场景下优于全参数微调，且参数量和计算成本降低两个数量级以上。
2. LoRA在视觉任务中需要较低的学习率（1e-3），这一发现对视觉领域的LoRA应用具有实践指导意义。
3. **Gate-LoRA在全部三个数据集上取得最优准确率**（86.85%/97.99%/76.78%），验证了共享门控混合设计（LoRA+SSF）比独立使用任一方法更有效。
4. 层级消融分析揭示了深层在视觉适应中的主导作用，为PEFT方法的设计提供了方向性指导。
